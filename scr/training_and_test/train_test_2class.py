# scr/training_and_test/train_test_2class.py
    ...
    for epoch in range(1, epochs + 1):
        ...
        with torch.no_grad():
            for batch in val_loader:
                images = batch['image']
                additional_features = batch['additional_features']
                labels = batch['labels'].float().unsqueeze(1)  # (B,1)
                ...
                # Predizioni: logit -> sigmoid -> soglia 0.5
...
def train_and_evaluate(subjects_train, features_train, subjects_test, features_test, device):
    ...
    plt.title('Confusion Matrix for 2-Class Classification (BCE)')
